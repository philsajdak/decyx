...
- `CLAUDE_MODELS`: List of available Claude models. By default, it uses `claude-sonnet-4-5-20250929` and will skip prompting for model selection if there is only one model in this list. This project was developed with the older model `claude-3-5-sonnet-latest` in mind, so if there are any issues, please use that model instead.
